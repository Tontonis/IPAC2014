...
A comparison of the time domain measurements of $S_{21}$ and the corresponding beam coupling impedance for each signal is shown in Fig.~\ref{fig:measComp}. The measurements with matching resistors, shown in *colour* show that there is a large reduction in multiple reflections when compared to the non-gated, non-matched signal in *colour*. The gated measurement, shown in *colour* can be seen to screen out many of the reflections also, however when the frequency domain data is processed to give a beam coupling impedance it can be seen that there is a large DC offset, caused by the loss of transmitted energy by the gating of the signal. The non-matched, non-gated signal is exceptionally noisy and provides no useful data, whereas the resistively matched measurements give good results below $\approx$ 500 MHz but the residual mismatch in the system (characteristic impedance of DUT $Z_{c}^{DUT}\approx 270$ was matched used a carbon film resistor of 220 $\Omega$) causes large oscillations which mask the true impedance. 

\begin{figure}
\subfigure[]{

\label{fig:timeDomMatchingETC}
}
\subfigure[]{

\label{fig:freqDomMatchingETC}
}

\caption{\subref{fig:timeDomMatchingETC} Time domain signal for different methods of eliminating reflected signals in the coaxial wire measurements. \subref{fig:freqDomMatchingETC} shows the resulting beam coupling impedance measurements.}
\label{fig:measComp}
\end{figure}

\section{Power Loss}

To determine the temperatures reached during operation of the MKI under various operational conditions of the LHC it is necessary to calculate the power lost by the beam into the MKIs due to beam-wakefield interactions. This is calculated using Eqn.~\ref{eqn:powLoss}

\begin{equation}
\P_{loss} = 2 e N_{b} n_{bunches} f_{rev} \
\end{equation}
